...
- Writing `results/pc_estimates.parquet` with columns `regime, condition, b, kappa, L, p_c, p_c_stderr, method`.
...
def resolve_p(p_rel: float, *, regime: str, condition: str, b: float, kappa: float,
    ...
    Raises if pc_estimates.parquet is missing or lacks the row. Never falls back."""
```
