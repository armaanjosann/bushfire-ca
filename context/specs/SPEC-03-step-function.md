...
- Computing derived metrics — `burned_fraction`, `reached_edge`, `spanned`, `settlement_reached` are SPEC-04's functions. `run_fire` may hold the raw counts and grids they need, but the summary functions live in `metrics.py`.
...
- `capture_scar=True` returns the final state grid. Default False, and callers must never request it for a sweep (§4.1).
