...
As `project-context.md` §4.2 — do not redesign. Parameters arrive through `**params`: `k` for `patches`, `w` for both strip conditions.
...
- [ ] `strips_perp` and `strips_para` at the same `w`, `b` and seed produce masks that are transposes of each other in shape statistics — same treated count within tolerance, orthogonal band orientation.
